...
fn cubic_bezier(x1: f32, y1: f32, x2: f32, y2: f32, t: f32) -> f32 {
    ...
    // Sample x at t
    let solve_x = |t: f32| ((ax * t + bx) * t + cx) * t;
    // Sample y at t
...
impl AnimationManager {
    ...
    pub fn get_animated_value(&self, node_id: usize, property: &str, current_time: f64) -> Option<AnimatableValue> {
        ...
        if let Some(trans) = self.transitions.get(&(node_id, property.to_string())) {
            ...
            return Some(trans.start_value.interpolate(&trans.end_value, eased_progress));
        }
        ...
        if let Some(anims) = self.animations.get(&node_id) {
            for anim in anims {
                 ...
                 let mut progress = (cycle % 1.0) as f32;
                 ...
                 if let (Some(s), Some(e)) = (start_val_str, end_val_str) {
                     if let (Some(v1), Some(v2)) = (AnimatableValue::parse(s), AnimatableValue::parse(e)) {
    ...
    pub fn start_transition(&mut self, node_id: usize, property: String, start: AnimatableValue, end: AnimatableValue, duration: f64, timing: TimingFunction, now: f64) {
    ...
    pub fn start_keyframe_animation(&mut self, node_id: usize, name: String, keyframes: Vec<super::css_values::CssKeyframe>, duration: f64, timing: TimingFunction, now: f64) {
