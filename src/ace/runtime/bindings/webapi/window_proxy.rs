...
impl WindowProxy {
    ...
    pub fn post_message<'js>(
        ...
        let message_json = match ctx.json_stringify(message)? {
            Some(v) => v.as_string().expect("Albedo Engine: internal invariant violated").to_string()?,
            None => "null".to_string(),
        };
        ...
        let (allowed, target_rt_arc) = {
            if let Some(arc) = crate::ace::runtime::core::registry::get_runtime(self.target_runtime_id) {
                let target_rt = arc.lock().unwrap_or_else(|e| e.into_inner());
        ...
        if let Some(arc) = target_rt_arc {
            ...
            let target_rt = arc.lock().unwrap_or_else(|e| e.into_inner());
            target_rt.event_loop.lock().unwrap_or_else(|e| e.into_inner()).enqueue_message(
                message_json,
                caller_origin,
                Some(caller_rt.id),
            );
