use rquickjs::{Class, Ctx, Result, Value, Object, Persistent, prelude::*};
...
impl URL {
    ...
    pub fn new(ctx: Ctx<'_>, input: String, base: Option<String>) -> Result<Self> {
        let url = if let Some(base_str) = base {
            let base_url = url::Url::parse(&base_str).map_err(|_| rquickjs::Error::new_from_js("URL", "Invalid base URL"))?;
            base_url.join(&input).map_err(|_| rquickjs::Error::new_from_js("URL", "Invalid URL parsing"))?
        } else {
            url::Url::parse(&input).map_err(|_| rquickjs::Error::new_from_js("URL", "Invalid URL"))?
        };
        ...
        let search_params_obj = <rquickjs::Value<'_> as Clone>::clone(&search_params_class).into_object();
    ...
    pub fn href_setter(&mut self, val: String) -> Result<()> {
        self.url = url::Url::parse(&val).map_err(|_| rquickjs::Error::new_from_js("URL", "Invalid URL"))?;
        Ok(())
    }
    ...
    pub fn host(&self) -> String {
        format!("{}{}", self.url.host_str().unwrap_or(""), self.url.port().map(|p| format!(":{}", p)).unwrap_or_default())
    }
    ...
    pub fn search_params<'js>(&self, ctx: Ctx<'js>) -> Result<Value<'js>> {
        self.search_params.clone().restore(&ctx).map(|obj: Object<'js>| obj.into_value())
    }
