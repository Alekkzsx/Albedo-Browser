...
async fn test_osr_named_function_loop() {
    ...
    let mut runtime = JsRuntime::new_with_jit(Arc::clone(&bridge), Arc::clone(&profiler), Arc::clone(&registry)).unwrap();
    ...
    let target_block = map.block_to_qjs_offset.iter()
        .find(|(_, &v)| v == 4)
        .map(|(&k, _)| k).expect("Loop header não mapeado");

    // Tentar compilar OSR para esse ponto
    let ptr = bridge.try_osr(&id, 4, &registry).expect("OSR compile falhou");
    assert!(!ptr.is_null());
