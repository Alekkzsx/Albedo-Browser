...
pub extern "C" fn js_div(lhs: u64, rhs: u64) -> u64 {
    let f1 = JsValue(lhs).to_number();
    let f2 = JsValue(rhs).to_number();
    JsValue::float64(f1 / f2).0
}
...
pub extern "C" fn js_bit_and(lhs: u64, rhs: u64) -> u64 {
    let a = JsValue(lhs).to_number() as i32;
    let b = JsValue(rhs).to_number() as i32;
    JsValue::int32(a & b).0
}
...
pub extern "C" fn js_bit_ushr(lhs: u64, rhs: u64) -> u64 {
    let a = JsValue(lhs).to_number() as u32;
    let b = JsValue(rhs).to_number() as i32;
