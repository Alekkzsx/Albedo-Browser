...
impl StackToRegisterTranslator {
    ...
    pub fn translate(mut self, func: QjsBytecodeFunction) -> (AirFunction, AirSourceMap) {
        ...
        for (i, op) in func.opcodes.iter().enumerate() {
            ...
            println!("OFFSET {}: {:?} (Current Blk: {})", offset, op, self.builder.current_block);
            ...
            if let Some(block_id) = self.jump_targets.get(&offset) {                
                ...
                if curr_idx != block_id.0 as usize && self.builder.blocks[curr_idx].terminator.is_none() {
    ...
    fn translate_instruction(&mut self, op: &QjsOpcode, qjs_offset: usize) {
        match op {
            ...
            QjsOpcode::PushString(str_idx) => {
                let s = self.const_pool_strings.get(*str_idx as usize)
                    .cloned()
            ...
            // --- Control Flow ---
            QjsOpcode::Return => {
                let ret_val = self.vstack.pop().unwrap_or_else(|| self.builder.emit_load_undefined());
            ...
            QjsOpcode::IfFalse(offset_jump) => {
                ...
                let then_block = *self.jump_targets.get(&fallthrough_offset).unwrap_or_else(|| {
            ...
            _ => { println!("[Decoder] Ignorando Opcode {:?}", op); }
        }
    }
}
