...
use albedo_jit::bytecode::{AirFunction, AirOpcode, AirTerminator, AirBlock, AirBlockId, AirReg, AirConstantPool};
use albedo_jit::js_value::JsValue;
use albedo_jit::builtins::BuiltinId;
use albedo_jit::tier2_compiler::Tier2Compiler;
use albedo_jit::type_feedback::{TypeFeedbackRegistry, IcKind};
...
fn test_fast_math_jit_stress() {
    ...
    b0.insts.push(AirOpcode::LoadFloat64 { dst: r_s, value: 0.0 });
    b0.insts.push(AirOpcode::LoadInt32 { dst: r_i, value: 0 });
    b0.insts.push(AirOpcode::LoadInt32 { dst: r_one, value: 1 });
    
    let abs_val = JsValue::builtin(BuiltinId::MathAbs as u64);
    let sqrt_val = JsValue::builtin(BuiltinId::MathSqrt as u64);
    b0.insts.push(AirOpcode::LoadFloat64 { dst: r_math_abs, value: f64::from_bits(abs_val.0) });
    b0.insts.push(AirOpcode::LoadFloat64 { dst: r_math_sqrt, value: f64::from_bits(sqrt_val.0) });
    b0.terminator = Some(AirTerminator::Jump(AirBlockId(1)));
    
    let mut b1 = AirBlock::new(1);
    b1.insts.push(AirOpcode::Lt { dst: r_cond, lhs: r_i, rhs: r_n });
    b1.terminator = Some(AirTerminator::JumpIf { cond: r_cond, then_blk: AirBlockId(2), else_blk: AirBlockId(3) });
    ...
    b2.insts.push(AirOpcode::Call { dst: r_abs, func: r_math_abs, arg_start: r_i, num_args: 1, ic_slot: s0 });
    b2.insts.push(AirOpcode::Call { dst: r_sqrt, func: r_math_sqrt, arg_start: r_abs, num_args: 1, ic_slot: s1 });
    b2.insts.push(AirOpcode::Add { dst: r_s, lhs: r_s, rhs: r_sqrt, ic_slot: s2 });
    b2.insts.push(AirOpcode::Add { dst: r_i, lhs: r_i, rhs: r_one, ic_slot: s3 });
...
fn test_fast_array_jit_stress() {
    ...
    b0.insts.push(AirOpcode::LoadFloat64 { dst: r_push, value: f64::from_bits(push_builtin.0) });
    b0.terminator = Some(AirTerminator::Jump(AirBlockId(1)));
    ...
    b2.insts.push(AirOpcode::Call { dst: r_len, func: r_push, arg_start: r_a, num_args: 2, ic_slot: s10 });
    b2.insts.push(AirOpcode::Add { dst: r_i, lhs: r_i, rhs: r_one, ic_slot: 99 }); // Slot dummy p/ add
    b2.terminator = Some(AirTerminator::Jump(AirBlockId(1)));
    ...
    air.const_pool = cp;
    b3.insts.push(AirOpcode::LoadString { dst: r_len_str, str_id: len_id });
    b3.insts.push(AirOpcode::GetProp { dst: r_len, obj: r_a, prop: r_len_str, ic_slot: s12 });
