...
impl AceTreeSink {
    ...
    fn apply_compat_fixups(&self, html: &str, children: &mut [HtmlNode], options: &ParserOptions) {
    ...
    fn inject_missing_table_newline(&self, nodes: &mut [HtmlNode]) -> bool {
        for node in nodes {
    ...
    fn mirror_selectedcontent_children(&self, nodes: &mut [HtmlNode]) {
        for node in nodes {
            if let HtmlNode::Element(element) = node {
    ...
    fn pick_selected_option_children(&self, children: &[HtmlNode]) -> Option<Vec<HtmlNode>> {
    ...
    fn populate_selectedcontent(&self, button: &mut HtmlElement, selected_children: &[HtmlNode]) -> bool {
