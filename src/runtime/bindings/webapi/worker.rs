use rquickjs::{Class, Ctx, Function, Result, Value, prelude::*, Object};
...
impl Worker {
    ...
    pub fn new(ctx: Ctx<'_>, url: String) -> Result<Self> {
        ...
        let parent_rt = Class::<JsRuntime>::from_object(rt_val.as_object().unwrap()).unwrap().borrow().clone();
        ...
        crate::runtime::core::registry::register_runtime(worker_rt_id, Arc::new(Mutex::new(rt.clone())));
        ...
        *rt.origin.lock().unwrap() = parent_rt.origin.lock().unwrap().clone();
            ...
            ctx.with(|ctx: Ctx<'_>| {
                ...
                let post_message = Function::new(ctx.clone(), move |msg: Value<'_>| {
                     ...
                     let msg_json = match m_ctx.json_stringify(&msg) {
                         Ok(Some(s)) => s.as_string().unwrap().to_string().unwrap_or_else(|_| "null".to_string()),
                         _ => "null".to_string(),
                     };
                     
                     if let Some(parent_arc) = crate::runtime::core::registry::get_runtime(parent_rt_id_captured) {
                         let parent_lock = parent_arc.lock().unwrap();
                         parent_lock.event_loop.lock().unwrap().enqueue_message(msg_json, "worker".to_string(), Some(worker_rt_id));
                     }
                }).unwrap();
        ...
        let resource_manager = parent_rt.resource_manager.lock().unwrap().clone();
                ...
                if let Ok(msg) = rx.try_recv() {
                    match msg {
                        WorkerMessage::PostMessage(json) => {
                             thread_rt.dispatch_message_event(json, "parent".to_string(), Some(parent_rt_id));
                        },
    ...
    pub fn onmessage_setter<'js>(&self, f: Function<'js>) {
